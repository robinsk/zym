...
class Zym_Controller_Action_Helper_MultiPageForm extends Zend_Controller_Action_Helper_Abstract
{
    ...
    {
        $formData = $this->getAllSubformData();
    ...
    {
        if (empty($this->_formData) && $this->getRequest()->isPost()) {
            $postData = $this->getRequest()->getPost();
            
            $elements = $this->getCurrentSubForm()->getElements();
            
            $this->_formData = array_intersect_key($postData, $elements);
        }
    ...
    public function handle()
    {
        ...
        $valid = $currentSubForm->isValid($this->getFormData());
        
        $this->setValues($currentSubForm, $valid);
        
        switch ($action) {
            case self::ACTION_KEY_BACK:
                $position = array_search($currentSubForm->getName(), $this->_actions);
                
                if ($position <= 0) {
                    $action = $this->_actions[0];
                } else {
                    $action = $this->_actions[$position - 1]; 
                }
                break;
            ...
            case self::ACTION_KEY_NEXT:
                ...
                if ($position == count($this->_actions) - 1) { 
